...
from .models import Medication, Cart, CartItem, Pharmacy, Order
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if quantity <= med.stock:
                add_med_to_cart(cart, med, quantity)
        ...
        if request.user.is_authenticated:
            if order_form.is_valid():
                ...
                if len(ord_nam) == 0:  # The order is unique
                    return save_order_redirect_to_thankyou(order_form.cleaned_data, cart)
...
def save_order_redirect_to_thankyou(clean_data, cart: Cart):
    order = Order(cart=cart, date_time=datetime.now(tz=timezone.utc))
